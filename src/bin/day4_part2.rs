...
fn calculate_bingo_score(data: Vec<&str>) -> u32 {
    ...
    let mut idx = 0;
    let mut row_index = 0;
    let mut boards_win_orders: Vec<u32> = Vec::new();
    let mut boards_completed = 0;
    ...
    'outer: for dn in draw_numbers {
        for (board_index, board) in boards.iter_mut().enumerate() {
            if !board.completed && board.verify_number(dn) {
                ...
                if board.completed && !boards_win_orders.contains(&(board_index as u32)) {
                    boards_completed += 1;
                    boards_win_orders.push(board_index as u32);
                }
                if boards_completed == boards_len {
                    break 'outer;
                }
            }
        }
    ...
    let last_board_index: usize = *boards_win_orders.last().unwrap() as usize;
    let last_board = boards.get(last_board_index).unwrap();
    let score = last_board.calculate_score();
